...
Optionally: if you want to run a local LLM that is compatible with the OpenAI API instead of OpenAI, make sure you set your environment variable like this before running the above steps:
  ...
  ```
You can use [LM Studio](https://lmstudio.ai/)'s API server found in the Development tab of the LM Studio app with the above approach. Note that you can set `OPENAI_API_KEY` to any value when using this approach as the auth token is completely ignored by LM Studio. Using a value like "dummy" is fine.
...
## Dry Run Mode
`push` is able to be run in "dry run" mode, meaning it will tell you what it would have done without actually committing or pushing. This is useful when you're not sure you'll like the AI-generated commit message or while making changes to `push` itself.

Simply add the `-d` flag to accomplish this, like so: `push -d` 
